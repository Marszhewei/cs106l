...
const M& HashMap<K, M, H>::at(const K& key) const {
    ...
    return static_cast<const M&>(node_found->value.second);
}
...
HashMap<K, M, H>::HashMap(const HashMap<K, M, H> &hm)
{
    ...
    this->_buckets_array.resize(hm.bucket_count());
    std::fill(_buckets_array.begin(), _buckets_array.end(), nullptr);
    for (auto curr : hm._buckets_array)
        while (curr != nullptr) {
            insert(curr->value);
            curr = curr->next;
        }
...
template <typename K, typename M, typename H>
HashMap<K, M, H> &HashMap<K, M, H>::operator=(const HashMap<K, M, H> &hm)
{
    if (this == &hm)
        return *this;
    clear();
...
{
    this->_size = hm._size;
    this->_hash_function = std::move(hm._hash_function);
    this->_buckets_array = std::move(hm._buckets_array);
}
