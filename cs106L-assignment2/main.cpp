...
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    ...
    int cnt = 0;
    for (auto &s: curr_set) {
        cnt += std::count_if(target_set.begin(), target_set.end(), [&s](string i)->bool{return i == s;});
    }
    return cnt; 
}
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    auto cmp_fn = [&w, &target_set](const vector<string>& left, const vector<string>& right) {
        ...
        // (void) left;
        // (void) right;
        return numCommonLinks(w.getLinkSet(left[left.size()-1]), target_set) < 
                                        numCommonLinks(w.getLinkSet(right[right.size()-1]), target_set);
